...
write_ready_xray_config(){
    ...
    # تم التعديل هنا لعدم استبدال إعدادات Xray الحالية إن وجدت (حفظ اليوزرات)
...
full_vpn_cleanup(){
    # تم الإبقاء على الدالة لعدم الإخلال ببنية السكربت، 
    # لكن تم إيقاف تفعيلها في الأسفل للحفاظ على بيانات المستخدمين
    echo "==================================="
    echo "   CLEANING OLD VPN/PROXY SERVICES"
    echo "==================================="
    # (محتوى الدالة كما هو في الأصل لكنه لن يعمل ولن يحذف المستخدمين)
}
...
setup_ready_ssl_ws_xhttp(){
    ...
    # تم التعطيل هنا للحفاظ على الإعدادات والمستخدمين السابقين
    # full_vpn_cleanup
...
# ==========================================================
# QUICK NETWORK PATH / ACCESS REPORT — DIAGNOSTIC ONLY
# ==========================================================
(
set +e

apt-get update -qq && apt-get install -y mtr-tiny >/dev/null 2>&1

echo "===== SERVER LOCATION/IP ====="
curl -4s https://ipinfo.io/json

echo
echo "===== SERVER BASE PING ====="
ping -c 10 1.1.1.1

echo
echo "===== SERVER ROUTE ====="
mtr -4 -rwzc 20 1.1.1.1
...
echo
echo "===== ACTIVE PROXY CHAIN ====="
ss -lntp | grep -E ':(22|80|443|8080)\b'
)

# ==========================================================
# BADVPN UDPGW & KERNEL TWEAKS INSTALLATION (APPENDED)
# ==========================================================
