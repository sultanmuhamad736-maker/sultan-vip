...
# Build: v33-VMESS-LINK V33-BASE + REAL-VMESS-LINK-MENU + REAL-PER-USER-LOGIN-STATS + LIVE-PRESERVED-XRAY + ORDERED-ONLINE-USERS + EXACT-N-LIVE-LINKS + LIVE-ONLINE-DECREMENT
...
touch "$DB" "$LIMIT_DIR/usage.db" "$LIMIT_DIR/blocked.db" "$LIMIT_DIR/quota-locked.db" "$LIMIT_DIR/devices.db" "$LIMIT_DIR/device_sessions.log" "$LIMIT_DIR/device_usage.db" "$XDB/usage.db" "$XDB/overview.db" "$XDB/user-online.db" "$XDB/online-sessions.db"
    ...
    case "$ANSWER" in
      y|Y)
        ...
        for DBF in "$XDB/vmess-ws.db" "$XDB/vless-ws.db" "$XDB/trojan-ws.db" \
                   "$XDB/usage.db" "$XDB/overview.db" "$XDB/user-online.db" "$XDB/online-sessions.db"; do
...
for TAG in vmess-ws vless-ws trojan-ws; do
  ...
  while IFS='|' read -r USER VALUE PROTO NET DOMAIN PATHX EXP QUOTA LOGIN CREATED REST; do
    ...
    if [ "$ONLINE_API_OK" = "1" ]; then
      # Count unique Xray IPs only when the same IP still has a live public
      # port-443 socket. A dead/replaced IP disappears instead of remaining in
      # the total and being counted again on every reconnect.
      ...
      # Some reverse-proxy/core combinations cannot expose the real client IP.
      # Fresh traffic is still authoritative proof that this account is online.
      if [ "$ONLINE_CURRENT" -eq 0 ] && [ "$DELTA" -gt 0 ]; then
        ONLINE_CURRENT=1
      fi
    else
      # Old cores without the bulk IP API are never trusted for an increasing
      # counter. Report one only when this account produced traffic in the
      # current scan; this prevents an old stale map from climbing forever.
...
touch "$DB" "$USAGE_DB" "$DEVICE_DB" "$DEVICE_USAGE_DB" "$DEVICE_SESSIONS" "$XRAY_USAGE_DB" "$XRAY_OVERVIEW_DB" "$XRAY_USER_ONLINE_DB" "$XRAY_ONLINE_SESSIONS_DB"
...
if not isinstance(outbounds, list) or not outbounds:
    cfg["outbounds"] = [{"tag": "direct", "protocol": "direct", "settings": {}}]

cfg["stats"] = cfg.get("stats") if isinstance(cfg.get("stats"), dict) else {}
cfg["api"] = {"tag": "api", "listen": "127.0.0.1:10090", "services": ["StatsService"]}
...
nginx_xray_alias_locations(){
  ...
  local FILE="$XDB/preserved-ws-aliases.db" TAG ALIAS CANON PORT
  [ -s "$FILE" ] || return 0
  ...
  while IFS='|' read -r TAG ALIAS; do
    ...
    cat <<EOF
    # Preserved legacy V2Ray link path. The client keeps the old path exactly.
    location = $ALIAS {
        rewrite ^ $CANON break;
...
write_nginx_config(){
  local D="$1" CERT="$2" KEY="$3" WS_TOKEN="$4" VLESS_PATH VMESS_PATH TROJAN_PATH XRAY_ALIAS_BLOCKS
...
remove_script(){
  ...
  rm -f /etc/sysctl.d/99-sultan-bbr.conf
  rm -f /etc/sysctl.d/98-sultan-online-detection.conf
  sysctl --system >/dev/null 2>&1 || true
